...
def _warp_image_volume(moving_image,
                       fixed_image,
                       moving_points,
                       transformed_points,
    ...
    """
    print(len(moving_points), len(transformed_points))
...
def warp_image_volume(
    ...
    # Get linked layers
    if len(linked_layers) > 0:
        img_wrp_list = []
        images = linked_layers
        ...
        for image in images:
            ...
            img_wrp_list.append(Image(img_wrp.data, image.name + '_warped'))
        print(img_wrp_list)
        return img_wrp_list
        ...
        print(f'Finished warping after {time.time() - start_time}s!')
